...
static DIR	*check_validate_cmd(t_prompt *p, t_list *cmd, char ***split_path)
{
	...
	if (full_cmd && ft_strchr(*full_cmd, '/') && !dir)
	{
		*split_path = ft_split(*full_cmd, '/');
		...
		current_cmd->full_path = find_cmd_in_path(p, current_cmd, split_path);
	return (dir);
}
...
static void	prepare_command(t_prompt *prompt, t_list *cmd)
{
	...
	current_cmd = cmd->content;
	dir = check_validate_cmd(prompt, cmd, &split_path);
	split_path = NULL;
